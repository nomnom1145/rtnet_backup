...
DEFAULT_REPORT_TYPE = "SHIN_server_commands"
HEADERS = ["Hostname", "IP", "Command", "Result", "실행결과"]
...
def is_truthy(value) -> bool:
    ...
    return normalized in {"1", "true", "yes", "y", "pass", "success", "양호"}


def resolve_action_required(entry) -> str:
    check_result = first_value(entry.get("check_result"))
    if check_result == "양호":
        return "성공"
    if check_result:
        return "실패"

    if is_truthy(entry.get("is_pass")) or is_truthy(entry.get("is_success")):
        return "성공"
        ...
        rows.append(
            {
                ...
                "action_required": resolve_action_required(entry),
            }
        )
...
def write_workbook(rows, output_path: str) -> None:
    ...
    need_fill = PatternFill(fill_type="solid", fgColor="FCE4D6")
    ...
    action_alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
    ...
    for row_index, row in enumerate(rows, start=2):
        ...
        for col, value in enumerate(values, start=1):
            ...
            cell.alignment = action_alignment if col == 5 else body_alignment
            cell.border = border

            if col == 5 and value == "실패":
                cell.fill = need_fill
...
def create_version_report(json_data, file_path: str) -> bool:
    try:
        ...
        display.display(f" 보고서가 생성되었습니다: {output_path}")
        return True

    except Exception as e:
        import traceback

        display.display(f" 보고서 생성 중 오류 발생: {e}")
        traceback.print_exc()
        return False

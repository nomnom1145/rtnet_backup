...
def copy_cell_style(ws, source_ref, target_ref):
...
def parse_process(lines):
    ...
    for line in lines:
        ...
        if not parts:
            continue
        name = parts[0]
        if name not in TARGET_PROCESS_NAMES or name in seen:
            continue
        status = parts[1] if len(parts) > 1 else ""
        result.append((name, normalize_process_status(status)))
        seen.add(name)
    return result
    ...
    if process_rows:
        for index, service_cell in enumerate(service_cells):
            if index < len(process_rows):
                name, status = process_rows[index]
                set_result_cell(ws, service_cell, name)
                set_result_cell(ws, status_cells[index], status)
            else:
                set_result_cell(ws, service_cell, "")
                set_result_cell(ws, status_cells[index], "")
    else:
        for index, service_cell in enumerate(service_cells):
            set_result_cell(ws, service_cell, MISSING_TEXT if index == 0 else "")
            set_result_cell(ws, status_cells[index], "")
...
def write_host_section(ws, host_data, section):
    ...
    process_rows = parse_process(clean_lines(host_data.get("process_output")))
    write_process_table(ws, section["process_table"], process_rows)

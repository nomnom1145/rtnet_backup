...
local_file = os.environ.get('script_path', '')
remote_file = os.environ.get('output_path', '')
...
# SCP 명령어 실행
command = f'scp -P{port} -o "HostKeyAlgorithms=+ssh-rsa" -o "HostKeyAlgorithms=+ssh-dss" -o "KexAlgorithms=+diffie-hellman-group-exchange-sha1" -o "KexAlgorithms=+diffie-hellman-group1-sha1" -o "KexAlgorithms=+diffie-hellman-group14-sha1" {local_file} {remote_user}@{remote_host}:{remote_file}'
try:
